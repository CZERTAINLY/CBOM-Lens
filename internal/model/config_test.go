			...
			then: []model.CueErrorDetail{
				{
					...
					Raw: "#Config.service: conflicting values null and {#ServiceFields,mode:(*\"manual\"|\"timer\"|\"discovery\"),schedule?:(null|#Schedule),if (mode == \"timer\") {schedule:#Schedule},dir?:string,repository?:#Repository,server?:(null|#LensServer),core?:(null|#Core),if (mode == \"discovery\") {server:#LensServer,core:#Core}} (mismatched types null and struct)",
				},
			},
		},
		{
			...
			then: []model.CueErrorDetail{
				{
					...
					Code:    model.CodeConflictingValues,
					Message: "Conflicting values for config: expected type list: got struct",
					Pos: model.CueErrorPosition{
						Filename: "config.yaml",
						Line:     8,
						Column:   5,
					},
					Raw: `#Config.containers.config: conflicting values {"-name":"c1"} and [...#ContainerConfig] (mismatched types struct and list)`,
				},
			},
		},
		{
