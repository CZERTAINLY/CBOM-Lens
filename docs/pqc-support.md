...
Listed explicitly, because an earlier version of this document implied support
that did not exist.
...
`1.3.9999.6.1.1`, `.2` and `.3` were previously in the registry labelled
HQC-128/192/256. They are not HQC: in `oqs-provider`'s
`oqs-template/generate.yml` those OIDs belong to SPHINCS+-Haraka-128f-robust
(NIST Round 3) and its p256 and rsa3072 hybrids. They have been removed, and a
...
still contains no post-quantum material. Adding one PQC detection to it is
tracked as follow-up work, to be done after the CycloneDX 1.7 emitter work
lands.
